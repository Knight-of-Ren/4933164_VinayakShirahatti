...
int* matchingStrings(int strings_count, char** strings, int queries_count, char** queries, int* result_count) {
    // The size of our result array will be the same as the number of queries.
    *result_count = queries_count;

    // Allocate memory for the integer array that will store the results.
    // The 'main' function in the template will handle freeing this memory.
    ...
    for (int i = 0; i < queries_count; i++) {
        ...
        // Loop through all the available strings to check for matches.
        ...
        // Store the final count for the current query in our results array.
        results[i] = count;
    }

    // Return the pointer to the results array.
    return results;
}
