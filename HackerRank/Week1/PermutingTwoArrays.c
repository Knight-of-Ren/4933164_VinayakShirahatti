...
char* twoArrays(int k, int A_count, int* A, int B_count, int* B) {
    ...
    // 3. Check if the sum of each pair meets the condition
    for (int i = 0; i < A_count; i++) {
        if (A[i] + B[i] < k) {
            // If we find any pair that fails, we know it's impossible
            return "NO";
        }
    }
    
    // If we get through the whole loop, a valid permutation exists
    return "YES";
}
